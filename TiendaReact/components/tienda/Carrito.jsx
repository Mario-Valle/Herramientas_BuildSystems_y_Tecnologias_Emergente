...
class Carrito extends React.Component{
    ...
    if(this.contadorCarrito()){
      return(
        ...
        <BarraNavegacion contador={this.contadorCarrito()}/>
          ...
          <hr></hr>
            <div className="row main">
              <div className="col-lg-5 col-md-5 col-sm-12 xx">
                      {  this.mostrarCarrito()}
              </div>
  ...
  mostrarCarrito(){
    return this.state.listaCarrito.map(
      (producto) => { return <CarritoDetalle key={ producto.id } id={producto.id}  nombre={ producto.nombre } imagen={ producto.imagen } informacion={ producto.informacion } cantidad={ producto.cantidad } precio ={producto.precio} actualizarDisponible={this.actualizarDisponible.bind(this)}/> }
    )
  }
  //==============================================================================
  ...
  pagarCarrito(){
    ...
    request
    .get('https://tienda-501bc.firebaseio.com/productos/.json')                //Realizar una consulta a la base de datos
    .then((res) => {
...
actualizarDB(itemCatalogo, cantidad){
  request.put(`https://tienda-501bc.firebaseio.com/productos/${itemCatalogo.id}.json`)  //Realizar consulta a base de datos
